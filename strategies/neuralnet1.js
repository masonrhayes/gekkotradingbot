...
var strategy = {
  ...
  init : function() {
    ...
    this.trainer = new convnetjs.SGDTrainer(this.nn, {
        // replace all this.settings with numbers from neuralnet.toml file
        // learning rate = 0.01, momentum = 0.1, decay = 0.01
      learning_rate: 0.01,
      momentum: 0.1,
      batch_size: this.batchsize,
      l2_decay: 0.01
      // end replacements here
    });

    this.addIndicator('stoploss', 'StopLoss', {
        // replace this.settings for stop loss threshold of 0.85
      threshold : 0.92
    });
        // and for hodl threshold
    this.hodl_threshold = 1.05 || 1;
  },
  ...
  update : function(candle)
  {
    ...
    if (2 > this.priceBuffer.length) return;
     ...
     for (tweakme=0;tweakme<5;++tweakme)
      this.learn();
    // replace price buffer with 100
    while (100 < this.priceBuffer.length) this.priceBuffer.shift();
  },
  ...
  check : function(candle) {
      // min predictions = 1000
    if(this.predictionCount > 1000)
    {
      if (
          'buy' === this.prevAction
          // stop loss enabled = false as default
          && true
      ...
      let signal = meanp < currentPrice;
        // threshhold buy = 1
      if ('buy' !== this.prevAction && signal === false  && meanAlpha> 1.0 )
      {
      ...
      // threshold sell = -1.0
      ('sell' !== this.prevAction && signal === true && meanAlpha < -1.0 && signalSell)
      {
